...
function bookFit() {
  ...
  var bookScale = Math.min(
    1,
    document.documentElement.clientWidth / BOOK_WIDTH,
  );
  var controlsScale = 1;
...
$(function () {
  ...
  // Recenter while the page is turning so the completed spread is never
  // briefly clipped by #book-stage's overflow boundary.
  ...
  mag.bind("turned", function (e, page, pageObj) {
    if (page == 2 || (page == 3 && $(this).data("done"))) {
      $(".dedicase").fadeTo(500, 1);
    } else {
      //$('.dedicase').hide();
    }
  ...
  function renderPageNumbers(view) {
    ...
    // Page 1 is already printed in the Sommaire artwork.
    $("#numPageleft").html(left >= 2 ? left : "");
    $("#numPageright").html(right >= 2 ? right : "");
  }
    ...
    if (!isValidEmailAddress(commandeEmail)) {
      ...
      $.post(
        ...
        function (data) {
          $("#commandeErreur").html("");
          $("#commandeErreur").html(
            '<br><span style="color:green">Message envoy&eacute; </span>',
          );
        },
      );
  ...
  $("#FormSenderRecrutement").live("click", function (e) {
    ...
    if (!isValidEmailAddress(email)) {
      $("#recrutementErreur").html(
        '<br><span style="color:red">Adresse EMAIL non valid</span>',
      );
      e.preventDefault();
    } else {
      $.post(
        "ajax/recrutement.php",
        {
          type: type,
          nom: nom,
          email: email,
  ...
  $("#FormSenderContact").live("click", function (e) {
    ...
    m_data.append(
      "typeContact",
      $("input[type=radio][name=typeContact]:checked").attr("value"),
    );
